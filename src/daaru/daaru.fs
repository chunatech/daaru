...
/// this is where daaru looks for the standalone fsi portion of the application 
let fsiSaLocation: string = (Path.Join(AppContext.BaseDirectory, "fsi_standalone/fsi_standalone"))

/// the staging directory location. see daaru.Transactions/cwTransactionsBuilder.fs
...
let main args = 
    ...
    let daaruLogFile: string = Path.Join(logDirectory, "daaru.log")

    logger.Log daaruLogFile (MethodBase.GetCurrentMethod()) Severity.Info $"application configuration received"
    logger.Log daaruLogFile (MethodBase.GetCurrentMethod()) Severity.Info $"transaction builder initialzied"
    logger.Log daaruLogFile (MethodBase.GetCurrentMethod()) Severity.Info $"logger initialized"
    ...
    logger.Log daaruLogFile (MethodBase.GetCurrentMethod()) Severity.Info $"program initialzied with settings: {appConfig}"
    ...
    logger.Log daaruLogFile (MethodBase.GetCurrentMethod()) Severity.Info $"transactions registered %s{existingScripts.ToString()}"
    ...
    logger.Log daaruLogFile (MethodBase.GetCurrentMethod()) Severity.Info $"watchers created for source and staging directories"

    // init TransactionRunner
    let runner: TransactionRunner = TransactionRunner.init fsiSaLocation appConfig logger daaruLogFile
    logger.Log daaruLogFile (MethodBase.GetCurrentMethod()) Severity.Info $"transaction runner initialized"
