    ...
    let createCSVRecordStr<'T> (record: 'T) =
        ...
        for prop: System.Reflection.PropertyInfo in props do
            ...
            let strValue: string =
                ...
                | :? System.DateTime as (dt: System.DateTime) -> CreateTimeStampNow dt TimeStampDateFormat
    ...
    let PartialRollFileBySize (filepath: string) (sizeLimitMB: int64) (leaveLines: int) (header: bool) =
        if File.Exists(filepath) then
            if FileInfo(filepath).Length >= sizeLimitMB * int64 (1024 * 1024) then
                ...
                let rolledFileContent: string array =
                    fileContent[0 .. (fileContent.Length - leaveLines - 1)]

                let mutable remainingFileContent: string array =
                    fileContent[(fileContent.Length - leaveLines) .. (fileContent.Length - 1)]
...
module FilePathDecoder =
    let Decoder: Decoder<string> =
        fun (path: string) (value: JsonValue) ->
            ...
            if
                not
                <| (System.Uri.IsWellFormedUriString(v.ToString(), System.UriKind.RelativeOrAbsolute))
            then
...
module DirPathDecoder =
    let Decoder: Decoder<string> =
        fun (path: string) (value: JsonValue) ->
            ...
            if
                (not
                 <| (System.Uri.IsWellFormedUriString(dirpath, System.UriKind.RelativeOrAbsolute)))
