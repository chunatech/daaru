...
daaru configuration is handled through a `config.json` file in the `config` directory of your daaru installation. 

None of these options are required to test out daaru. daaru will run 
...
daaru is run from and that is expected to be the daaru installation 
...
maxThreadCount | Optional | how many threads daaru transaction runner may use to run transactions in parallel
...
This option configures what directoried daaru will watch and pull transaction files from. The files contain `.dt` 
extensions represent the tests to be run. They are written in FSharp and composed into full `.fsx` files to be run 
by daaru. The composed files will go into the `staging` directory of the application, and the directory structure will  
...
This option configures how many threads daaru will use for running transactions. The transactions run in parallel. 
...
This configures how frequently daaru will poll for transactions to run. The time is given in `minutes`. 
...
"resultsDir": string
```
The path for `{transactionfilename}_results.csv` files generated by daaru. See Transaction Results Publishing for more 
details on what data daaru publishes. 
...
Browsers is an array of `BrowserConfiguration` objects. daaru is currently only set up to handle the first 
...
At this time daaru only supports chrome. The user needs to obtain the appropriate chrome driver for their 
...
daaru will add these into the transactions when composing them. 
...
The path to where the user would like to have the logs. If this option is ommited, daaru will use the 
default log path. 
...
daaru rolls log files after they reach a limit size.  The file size limit confguration option allows the 
user to specify in `MB` how large a file should be before daaru rolls it. the format for a rolled log 
file is as follows 
...
daaru has multiple levels of logging that can be configured ranging from `Debug` to `Critical`. The range 
...
daaru can be configured to retreive credentials for use in tranasactions if the user provides a path to the credential request script and 
...
```
daaru uses canopy for much of its functionality. These options are canopy 
configurations that haven't been accounted for in other settings. daaru will 
